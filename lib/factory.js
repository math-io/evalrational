...
function factory( P, Q ) {
	...
	// If more than one coefficient, apply Horner's method to both the numerator and denominator...
		...
		// If `ax <= 1`, evaluate the numerator and denominator of the rational function using Horner's method...
		f += 'if(ax<=1){';
		f += 's1 = ' + P[ 0 ];
		m = n - 1;
		for ( i = 1; i < n; i++ ) {
		...
		// If `ax > 1`, evaluate the numerator and denominator via the inverse to avoid overflow...
		f += 'x = 1/x;';
		m = n - 1;
		f += 's1 = ' + P[ m ];
		for ( i = m - 1; i >= 0; i-- ) {
